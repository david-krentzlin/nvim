...
vim.g.clipboard   = 'osc52'        -- Use OSC 52 for explicit `+` clipboard access
vim.o.clipboard   = ''             -- Keep default yank and paste in unnamed register
...
Config.should_trim_trailing_whitespace = function(bufnr)
  bufnr = bufnr or vim.api.nvim_get_current_buf()
  if vim.bo[bufnr].buftype ~= '' or not vim.bo[bufnr].modifiable or vim.bo[bufnr].readonly then
    return false
  end

  local lines = vim.api.nvim_buf_get_lines(bufnr, 0, math.min(vim.api.nvim_buf_line_count(bufnr), 5), false)
  for _, line in ipairs(lines) do
    if line:find('@generated', 1, true) or line:match('[Cc]ode generated.-[Dd][Oo] [Nn][Oo][Tt] [Ee][Dd][Ii][Tt]') then
      return false
    end
  end
  return true
...
Config.trim_trailing_whitespace = function(bufnr)
  bufnr = bufnr or vim.api.nvim_get_current_buf()
  if not Config.should_trim_trailing_whitespace(bufnr) then return end
  ...
  if changed then vim.api.nvim_buf_set_lines(bufnr, 0, -1, false, lines) end
end

Config.new_autocmd('BufWritePre', nil, function(ev) Config.trim_trailing_whitespace(ev.buf) end, 'Trim trailing whitespace')
